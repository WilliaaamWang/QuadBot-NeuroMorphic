...
class SynapticNeuron():
    ...
    def V_dot(self, V, Vs, Vus, I_ext, # Current values
        ...
        #! TODO
        return self.k * (I_ext + g_f*((V-V0)**2) - g_s*((Vs-Vs0)**2) - g_us*((Vus-Vus0)**2) + np.sum(Ie) + np.sum(Ii)) / C
...
#! TODO
# Backward Euler integration -- more stable?
...
def simulate_euler(num_steps, dt, 
    ...
    for i in range(0, num_steps-1):
        ...
        Se_new = exponent_euler(Se_i, dt, lambda x, y: neuron.Se_dot(Se_i, Ve_delayed, Ve_threshold, tau_e))

        Si_new = exponent_euler(0, Si_i, dt, lambda x, y: neuron.Si_dot(Si_i, Vi_delayed, Vi_threshold, tau_i))

        V_new = exponent_euler(0, V_i, dt, lambda x, y: neuron.V_dot(V_i, Vs_i, Vus_i, I_i, V0, Vs0, Vus0, g_f, g_s, g_us, C, Se_i, Si_i, Ve0, Vi0, g_syn_e, g_syn_i))
